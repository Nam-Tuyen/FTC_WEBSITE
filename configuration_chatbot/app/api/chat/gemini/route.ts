...
function initGemini() {
  ...
  try {
    ...
    throw new Error(`Failed to initialize Gemini: ${e instanceof Error ? e.message : 'Unknown error'}`);
  }
}
...
async function loadKnowledgeBase() {
  ...
  try {
    ...
    try {
      ...
      return 'Thông tin về câu lạc bộ FTC chưa có sẵn.';
    }
  ...
  // If no knowledge base files found, provide basic information
  if (parts.length === 0) {
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error('[api/chat/gemini] GEMINI API not configured:', error)
    ...
    // Load knowledge base using RAG system
    let knowledgeContext = '';
    try {
      knowledgeContext = await ragSystem.buildContextString(message);
      console.log('[api/chat/gemini] Loaded knowledge context, length:', knowledgeContext.length);
    } catch (error) {
      console.warn('[api/chat/gemini] Failed to load knowledge base:', error);
...
TRẢ LỜI:`;
    ...
    // Determine if it's a configuration error or server error
    const isConfigError = error.message?.includes('GEMINI_API_KEY') || 
                         error.message?.includes('Failed to initialize Gemini');
    
    return new Response(JSON.stringify({
      error: true,
      message: isConfigError ? 'Service configuration error' : 'Internal server error',
      response: isConfigError 
        ? 'Xin lỗi, dịch vụ AI chưa được cấu hình đúng cách. Vui lòng liên hệ quản trị viên.'
