...
async function loadKnowledgeBase() {
  ...
  try {
    ...
    // Load structured knowledge base first
    try {
    ...
    // Load unstructured content as fallback
...
export async function POST(req: Request) {
  try {
    ...
    const mode = body?.mode === 'domain' ? 'domain' : 'club'
    console.log('[api/chat/gemini] incoming', { messageLen: message?.length || 0, historyLen: history.length, mode })
...
- ${mode === 'domain' ? 'Chuyên nghiệp nhưng không quá học thuật' : 'Thân thiện, phù hợp với tân sinh viên'}
- Tập trung vào thông tin thực tế và hữu ích
${mode === 'domain' ? '- Đề xuất tài liệu tham khảo thêm nếu phù hợp' : '- Kết thúc bằng lời mời tham gia CLB nếu phù hợp'}

Câu hỏi: ${message}

Trả lời:`;    // helper to extract text from various SDK response shapes
    ...
    try {
      const suggestionPrompt = mode === 'domain' ?
        `Bạn là chuyên gia fintech. Dựa trên câu hỏi và câu trả lời trên, hãy đề xuất 5 câu hỏi liên quan về chủ đề fintech/tài chính số mà người dùng có thể quan tâm. Trả về một mảng JSON thuần gồm các chuỗi.` :
        `Bạn là cố vấn thân thiện cho tân sinh viên. Dựa trên thông tin từ knowledge base (nếu có) và câu trả lời vừa soạn ở trên, hãy tạo 5 câu hỏi gợi ý về CLB mà tân sinh viên có thể tiếp tục hỏi. Trả về một mảng JSON thuần gồm các chuỗi.`;
    ...
    return new Response(JSON.stringify({
      ...
      answer: answer,
      response: answer,
      source: mode === 'domain' ? 'ai_expert' : 'knowledge_base',
      suggestions
    }), {
      headers: { 'Content-Type': 'application/json' }
    })
