      ...
      <div className="max-w-3xl mx-auto p-4 space-y-4">
        <header className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <>
                <span className="text-sm text-white/70">Xin chào, {user.full_name} ({user.mssv})</span>
        ...
        {tab==="login" && <div className="border rounded p-4 bg-white/10 backdrop-blur-xl"><LoginForm /></div>}
        {tab==="register" && <div className="border rounded p-4 bg-white/10 backdrop-blur-xl"><RegisterForm /></div>}
        ...
        {loading ? <p>Đang tải...</p> : (
          <div className="grid gap-3">
            ...
            {items.length===0 && <p className="text-white/70">Chưa có câu hỏi.</p>}
          </div>
        )}
      </div>
    </div>
