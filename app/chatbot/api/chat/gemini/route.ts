// app/chatbot/api/chat/gemini/route.ts
...
export async function POST(req: Request) {
  try {
    const { prompt, history } = (await req.json()) as { prompt: string; history?: HistItem[] }
    ...
    if (faqHit) {
      return new NextResponse(JSON.stringify({ text: faqHit }), { status: 200, headers: baseHeaders })
    }
    ...
    const text: string =
      data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim?.() ?? getBotFallbackAnswer(prompt)

    return new NextResponse(JSON.stringify({ text }), { status: 200, headers: baseHeaders })
  } catch (e) {
